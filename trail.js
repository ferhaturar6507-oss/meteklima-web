...
class ImageTrail {
    constructor(container) {
        ...
        const triggerEl = document.getElementById('mood-section') || container;
        const handlePointerMove = ev => {
            const rect = triggerEl.getBoundingClientRect();
            this.mousePos = getLocalPointerPos(ev, rect);
        };
        triggerEl.addEventListener('mousemove', handlePointerMove);
        triggerEl.addEventListener('touchmove', handlePointerMove);
        const initRender = ev => {
            const rect = triggerEl.getBoundingClientRect();
            this.mousePos = getLocalPointerPos(ev, rect);
            this.cacheMousePos = { ...this.mousePos };
            this.rafId = requestAnimationFrame(() => this.render());
            triggerEl.removeEventListener('mousemove', initRender);
            triggerEl.removeEventListener('touchmove', initRender);
        };
        triggerEl.addEventListener('mousemove', initRender);
        triggerEl.addEventListener('touchmove', initRender);
